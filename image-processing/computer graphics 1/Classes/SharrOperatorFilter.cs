...
namespace computer_graphics_1
{
    class SharrOperatorFilter : MatrixFilter
    {
        ...
        protected float[,] kernelY1 = null;
        public SharrOperatorFilter()
        {
            kernel = new float[3, 3] { { 3, 0, -3 }, { 10, 0, -10 }, { 3, 0, -3 } };
            kernelY = new float[3, 3] { { 3, 10, 3 }, { 0, 0, 0 }, { -3, -10, -3 } };
            kernel1 = new float[3, 3] { { -1, -1, -1 }, { 0, 0, 0 }, { 1, 1, 1 } };
            kernelY1 = new float[3, 3] { { -1, 0, 1 }, { -1, 0, 1 }, { -1, 0, 1 } };
        }
        ...
        protected override Color calculateNewPixelColor(Bitmap sourceImage, int x, int y)
        {
            ...
            for (int l = -radiusY; l <= radiusY; l++)
                for (int k = -radiusX; k <= radiusX; k++)
                {
                    ...
                    resultR += neighborColor.R * kernel1[k + radiusX, l + radiusY] + neighborColor.R * kernelY1[k + radiusX, l + radiusY];
                    resultG += neighborColor.G * kernel1[k + radiusX, l + radiusY] + neighborColor.G * kernelY1[k + radiusX, l + radiusY];
                    resultB += neighborColor.B * kernel1[k + radiusX, l + radiusY] + neighborColor.B * kernelY1[k + radiusX, l + radiusY];
                }
